// Referenced from : excerpts from [https://codesandbox.io/s/material-demo-33l5y]
...
const CourseSelect: React.FC<CourseSelectProps> = ({ assignedColors, handleSelect, handleRemove }) => {
  const [options, setOptionsState] = useState<CoursesList>([]); // Represents the list of courses displayed in the component.
  const [inputValue, setInputValue] = useState<string>(''); // Represents the current value entered in the search input field.
  const [selectedValue, setSelectedValue] = useState<CoursesList>([]); // Represents the list of courses that have been selected.
  const searchTimer = useRef<number | undefined>(); // A ref to store a timeout ID used for delaying search updates.
  const listRef = useRef<VariableSizeList | null>(null); // A ref to store a reference to the VariableSizeList component.

  // The component utilizes the useContext hook to access the coursesList and selectedCourses values from the AppContext and CourseContext contexts, respectively.
  const { coursesList } = useContext(AppContext);
  const { selectedCourses } = useContext(CourseContext);

  // An fuzzy object is created using the Fuse library, which allows for fuzzy search functionality. 
  // It is initialized with coursesList and searchOptions, and this effect runs whenever coursesList changes.
  useEffect(() => {
    fuzzy = new Fuse(coursesList, searchOptions);
  }, [coursesList]);

  /**
  * @desc Generate a list of the user's selected courses 
  * Another effect runs when selectedCourses or coursesList changes. 
  * It generates a list of selected courses by mapping the course codes from selectedCourses to their corresponding CourseOverview objects from coursesList. 
  * It filters out any undefined values and updates the selectedValue state with the filtered array.
  */
  useEffect(() => {
  ...
  /**
   * @desc Accepts a courseCode and returns the area of study of that course (substring of the code).
  ...
  //  It is used to determine whether the label of the input field should shrink or not.

...
const areIdenticalEvents = (curr: CreatedEvents, next: CreatedEvents) => {
  const sameTime = (a: EventTime, b: EventTime) =>
...
const extractHistoryInfo = (
  ...
  const courses =
    timetableActions[timetableId][actionsPointer[timetableId]].courses;
  const classes = duplicateClasses(
    timetableActions[timetableId][actionsPointer[timetableId]].classes
  );
  const events =
    timetableActions[timetableId][actionsPointer[timetableId]].events;
...
const areIdenticalTimetables = (
  ...
  return (
    ...
    areIdenticalClasses(
      currentActions[currentPointer].classes,
      selectedClasses
    ) &&

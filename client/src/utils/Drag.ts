...
export const getDefaultShadow = (isSquareEdges: boolean) =>
  isSquareEdges ? 0 : 3; // How much shadow each card should have by default
 ...
 */
export const isScheduledPeriod = (
  data: ClassCard | EventPeriod | null
): data is ClassPeriod =>
 ...
 */
const getInventoryPeriod = (
  courseData: CourseData,
  cardData: ClassCard
): InventoryPeriod => courseData.inventoryData[cardData.activity];
 ...
 */
const equalDur = (p1: ClassPeriod, p2: ClassPeriod) =>
  p1.time.end - p1.time.start === p2.time.end - p2.time.start;
...
export const checkCanDrop = (a: ClassCard | null, b: ClassCard | null) => {
  ...
  return (
    ...
    (!isScheduledPeriod(a) ||
      ...
      classData.periods.every(
        (p, i) => i === 0 || equalDur(p, classData.periods[i - 1])
      ))
  );
};
...
const getIntersectionArea = (drag: DOMRect, drop: DOMRect) => {
  ...
  const bottom = Math.min(
    drag.bottom + timetableBorderWidthPlusOne,
    drop.bottom
  );
...
const getIsElevated = (cardData: ClassCard | EventPeriod) => {
  if (cardData.type !== "event" && dragTarget?.type !== "event") {
    ...
    return (
      dragTarget !== null && (cardData === dragTarget || isMatchingClasses)
    );
  } else {
  ...
  Array.from(cards.entries()).forEach(([cardData, element]) => {
    ...
    if (isElevated) {
      ...
      element.style.zIndex = String(
        Number(element.style.zIndex) - elevatedZIndexOffset
      );
    }
...
export const useDrag = (
  selectHandler: ClassHandler,
  removeHandler: ClassHandler
) => {
...
const updateDropTarget = (now?: boolean) => {
  ...
  const bestMatch = Array.from(dropzones.entries())
    ...
    .map(([classPeriod, dropElement]) => {
      let area = dragElement
        ? getIntersectionArea(dragRect, dropElement.getBoundingClientRect())
        : 0;

      // Avoids card oscillating between inventory and timetable (because moving between inventory changes card height)
      if (
        dropElement === inventoryElement &&
        area < inventoryDropIntersection * dragRect.width * dragRect.height
      )
        area = 0;
  ...
  const newDropTarget =
    result !== null ? result : getInventoryPeriod(dragTargetCourse, dragTarget);
 ...
 * @returns
 */
export const morphCards = (
  a: ClassCard[] | EventPeriod[],
  b: ClassCard[] | EventPeriod[]
) => {
  const from = [...a];
  let to = [...b];

  const result: (ClassCard | EventPeriod | null)[] = Array(from.length).fill(
    null
  );
  ...
  if (
    ...
    from.includes(dragTarget) &&
    to.includes(dropTarget)
  ) {
    ...
    if (to.includes(fromCard)) {
      ...
      if (fromElement) {
        const closest = (to as ClassPeriod[])
          ...
          .map((toCard) => {
            const element = isScheduledPeriod(toCard)
              ? dropzones.get(toCard)
              : undefined;
            const distance = element
              ? distanceBetween(fromElement, element)
              : Infinity;
  ...
  (entries: ResizeObserverEntry[]) => {
    ...
    if (prevWidth !== -1) {
      ...
      moveElement(
        ...
        (prevWidth - entryWidth) / 2,
        (prevHeight - entryHeight) / 2
      );
    }
  ...
  if (cardData !== dragTarget) {
    ...
    if (cardData && event && event.currentTarget && scrollElement) {
      ...
      if (typeof event.pageX === "number" && typeof event.pageY === "number") {
        ...
        typeof event.touches === "object" &&
        typeof event.touches[0] === "object"
      ) {
        // On mobile
  ...
  (event: TouchEvent) => {
    ...
    if (scrollElement) {
      if (event.touches.length > 0) {
        onMove(
          event.touches[0].pageX + scrollElement.scrollLeft,
  ...
  if (dragElement) {
    ...
    if (dragTarget?.type === "event") {
      ...
      if (gridChildren && dragTarget) {
        ...
        const itemRect =
          gridChildren[
            Math.floor(gridChildren.length / 2)

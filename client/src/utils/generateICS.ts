...
export const downloadIcsFile = async (
  ...
  const formattedClassEvents = getClassEvents(courses, classes).map(
    ([period, week]) => ({
      start: generateDateArray(
        firstDayOfTerm,
        timezone,
        period.time.start,
        period.time.day,
  ...
  const formattedCreatedEvents = getCreatedEvents(createdEvents).map(
    ([period, week]) => ({
      start: generateDateArray(
        firstDayOfTerm,
        timezone,
  ...
  const icsFile = createEvents(
    formattedClassEvents.concat(formattedCreatedEvents)
  );
  saveAs(
    new Blob([icsFile.value as BlobPart], { type: "text/ics" }),
    "my_timetable.ics"
  );
};
...
const getUtcOffset = async () => {
  try {
    const timezoneFetch = await fetch(
      "https://worldtimeapi.org/api/timezone/Singapore"
    );
...
const getClassEvents = (
  ...
  const allClasses = courses.flatMap((course) =>
    Object.keys(course.activities)
      .filter(
        (possibleActivity) =>
          classes[course.code] !== null &&
          classes[course.code][possibleActivity] !== null
      )
  ...
  return allClasses.flatMap((classTime) =>
    ...
    classTime!.periods.flatMap((period) =>
      period.time.weeks.map((week) => [period, week] as [ClassPeriod, number])
    )
  );
};
...
const getCreatedEvents = (
  ...
  return Object.entries(createdEvents).flatMap(([_, period]) =>
    [1, 2, 3, 4, 5, 7, 8, 9, 10].map(
      (week) => [period, week] as [EventPeriod, number]
    )
  );
};

...
def get_data(
    year: Union[str, int], semester: Union[str, int], ug_gd: str, code: str
) -> Dict[str, Optional[Union[str, ClassDict]]]:
    ...
    for i in range(ROUNDS):
        ...
        cursor = conn.execute(f"SELECT * FROM {TABLE_NAME} WHERE Code=?", (code,))
        ...
        for row in ROWS:
            ...
            if CLASSNAME in class_dict:
                class_dict[CLASSNAME].extend(
                    [BLANK for _ in range(i - len(class_dict[CLASSNAME]))]
                )
...
if __name__ == "__main__":
    print(get_data(20222023, 2, "UG", "EE2211")["classes"])
